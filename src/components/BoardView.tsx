    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-6 animate-fade-in">
      ...
      <div className="w-full glass-panel p-5 rounded-2xl border-game-neonCyan/30 flex items-center justify-between shadow-[0_0_20px_rgba(0,240,255,0.1)]">
        ...
        <div className="hidden md:flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-xl text-xs text-white/60">
          ...
          Hãy chọn một ô số dưới đây để bắt đầu câu hỏi!
        </div>
      </div>
            ...
            {questions.map((q) => {
              ...
              return (
                <button
                  ...
                  className={`aspect-square rounded-2xl flex flex-col items-center justify-center p-3 transition-all duration-300 relative overflow-hidden group border ${isUsed
                ...
                >
                  {isUsed ? (
                    <>
                      <Lock className="w-6 h-6 text-white/10 mb-1" />
                      <span className="text-lg font-black tracking-wider opacity-20">{q.letter}</span>
                      <span className="absolute bottom-2 text-[8px] font-extrabold text-white/10 tracking-widest uppercase">ĐÃ KHÓA</span>
                    </>
                  ) : (
                    <>
                      <span className="text-5xl font-black tracking-wider group-hover:scale-110 transition-transform duration-300">
                        {q.letter}
                      </span>
            ...
            <div className="space-y-2.5">
              {teams.map((team, idx) => {
                ...
                return (
                  <div
                    key={team.id}
                    className={`p-3.5 rounded-xl border flex items-center justify-between transition-all duration-300 ${isActive
